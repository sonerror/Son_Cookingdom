...
namespace AnhPD.Tanghulu
{
  public class TanghuluData : FoodData
  {
    ...
    {
      // Kiểm tra kiểu rõ ràng
      TanghuluData o = other as TanghuluData;
      if (o == null)
        return false;

      // So sánh danh sách fruits
      if (fruits.Count != o.fruits.Count)
        return false;

      for (int i = 0; i < fruits.Count; i++)
      {
        if (fruits[i] != o.fruits[i])
          return false;
      }

      // So sánh các thuộc tính còn lại
