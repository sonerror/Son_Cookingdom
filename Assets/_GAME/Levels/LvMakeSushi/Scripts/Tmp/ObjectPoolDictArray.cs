...
namespace Utilities
{
    ...
    /// </summary>
    public class ObjectPoolDictArray : Singleton<ObjectPoolDictArray>
    {
        ...
        {
            int key = prefab.GetInstanceID();
            List<GameObject> goList1 = _pooledGoDic[key];
            GameObject go = null;
        ...
        public GameObject GetGameObject(GameObject prefab, Vector3 position, Quaternion rotation, bool forceInstantiate = false)
        {
            ...
            {
                for (int i = goList1.Count - 1; i >= 0; i--)
                {
                    go = goList1[i];
                    if (go == null)
                    {
        ...
        public List<GameObject> GetGameObjects(GameObject prefab, int amount, bool forceInstantiate = false)
        {
            ...
            if (!forceInstantiate)
            {
                for (int i = goList1.Count - 1; i >= 0 && listReadyGameObject.Count < amount; i--)
                {
                    go = goList1[i];
                    if (go == null)
                    {
            ...
            goList1.Capacity = Mathf.Max(goList1.Capacity, goList1.Count + (amount - listReadyGameObject.Count));
        ...
        public int GetActivePooledObjectCount()
        {
            ...
            for (int i = 0; i < _pooledKeyList.Count; i++)
            {
                ...
                var goList1 = _pooledGoDic[key];
                for (int j = 0; j < goList1.Count; j++)
                {
                    var go = goList1[j];
